    ...
    def search(cell_x ,cell_y)
        ...
        if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
          ...
          if (@columns[cell_x][cell_y].north) != nil
            if (@columns[cell_x][cell_y].north).vacant
                north_cell = @columns[cell_x][cell_y].north
                north_path = cell_x,cell_y - 1
            end
          end
          if (@columns[cell_x][cell_y].south) != nil
            if (@columns[cell_x][cell_y].south).vacant 
              south_cell = @columns[cell_x][cell_y].south
                south_path = cell_x,cell_y + 1
            end
          end
          if (@columns[cell_x][cell_y].east) != nil
            if (@columns[cell_x][cell_y].east).vacant
              east_cell = @columns[cell_x][cell_y].east
                east_path = cell_x + 1,cell_y
            end
          end
          if (@columns[cell_x][cell_y].west) != nil
            if (@columns[cell_x][cell_y].west).vacant
              west_cell = @columns[cell_x][cell_y].west
          ...
          if (north_path != nil && !north_cell.visited)
            path = north_path
            path_cell = north_cell
          elsif (east_path != nil && !east_cell.visited)
            path = east_path
            path_cell = east_cell
          elsif (south_path != nil && !south_cell.visited)
            path = south_path
            path_cell = south_cell
          elsif (west_path != nil && !west_cell.visited)
            path = west_path
            path_cell = west_cell
          end
