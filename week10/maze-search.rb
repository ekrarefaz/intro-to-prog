  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if ((@columns[cell_x][cell_y].north).vacant) 
        north_path = @columns[cell_x][cell_y].north
      end
    
      if ((@columns[cell_x][cell_y].south).vacant)
        south_path = @columns[cell_x][cell_y].south
      end
    
      if ((@columns[cell_x][cell_y].east).vacant)
        east_path = @columns[cell_x][cell_y].east
      end
    
      if ((@columns[cell_x][cell_y].west).vacant)
        west_path = @columns[cell_x][cell_y].west
      end
      ...
      if (path != nil)
        ...
        (@colums[cell_x][cell_y]).concat(path)
      else
        if (ARGV.length > 0) # debug
